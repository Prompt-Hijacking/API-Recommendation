...
def read_and_filter_data(data_path):
    ...
    return data.filter(items=["bimodal", "target_api"])
...
# 添加 --output_dir 参数
parser.add_argument("--output_dir", type=str, default="./bimodal", help="Output directory for saving logs and models.")
args = parser.parse_args()

# 使用 args.output_dir 构建 log_file 路径
log_file = os.path.join(args.output_dir, f"bimodal_training_log.txt")
...
if __name__ == "__main__":
    ...
    if args.peft_config_type == "PrefixTuning":
        ...
        peft_config = PromptTuningConfig(task_type=TaskType.SEQ_2_SEQ_LM, prompt_tuning_init=PromptTuningInit.TEXT, num_virtual_tokens=20,
                                        prompt_tuning_init_text="Recommend a Java API sequence based on the following code",
